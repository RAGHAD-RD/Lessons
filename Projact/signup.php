...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!preg_match('/^(73|77|78)\d{7}$/', $phone_number)) {
    ...
    if (!isset($error)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("INSERT INTO users (email, password, phone_number, gender) VALUES (?, ?, ?, ?)");
            $stmt->execute([$email, $hashed_password, $phone_number, $gender]);

            // تخزين اسم المستخدم بعد التسجيل
